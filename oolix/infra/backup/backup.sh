...
#   2. The signing keys   — the private keys behind every manifest
#
...
#
# Treat the output like the live system. The dump holds every password hash
# and the archive holds the keys that sign sign-in tokens.
#
#   ./backup.sh /var/backups/oolix
#
# Environment (all optional, sensible defaults for the compose stack):
#   COMPOSE_FILE   default oolix/infra/docker/compose.prod.yml
...
COMPOSE_FILE="${COMPOSE_FILE:-oolix/infra/docker/compose.prod.yml}"
...
# half of it.
echo "  pg_dump $APPDB"
$DC exec -T postgres pg_dump -U "$PGUSER" -d "$APPDB" --format=custom --no-owner \
  > "$OUT/$APPDB.dump"
...
if [ ! -s "$OUT/$APPDB.dump" ]; then
  echo "  FAILED: $APPDB.dump is empty" >&2
  exit 1
fi
