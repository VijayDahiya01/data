...
func TestTheGuideExampleParses(t *testing.T) {
	// partner/agent/internal/config -> partner/, then the pack beside the agent.
	// They sit together on purpose: the guide documents this binary's config,
	// and a test that reads one against the other only means something if a
	// Partner receives them together.
	guide := filepath.Join("..", "..", "..", "pack", "INTEGRATION-GUIDE.md")
