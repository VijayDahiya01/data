...
| `agent-config.example.yaml` | [`../agent/config.example.yaml`](../agent/config.example.yaml) | live      |
...
| `mock-partner/`             | [`../mock-partner`](../mock-partner)                           | live      |
...
- **mirror** — `seed-data.yaml` describes the fixtures that
  `oolix/packages/db/prisma/seed/index.ts` creates. The identifiers are checked
