-- =============================================================================
-- Oolix control plane -- representative first migration (Appendix A, §96).
--
-- SNAPSHOT, NOT SOURCE OF TRUTH. §96 is explicit: "Prisma migrations remain
-- authoritative, but the following critical SQL is included inline so a
-- developer is not forced to open another file to understand core
-- constraints."
--
-- This file is a byte-for-byte copy of
--   oolix/packages/db/prisma/migrations/20260822122358_init/migration.sql
-- and `node scripts/verify-pack.mjs` fails if the two ever diverge. Edit the
-- Prisma schema and generate a migration; never edit this file by hand.
--
-- What to look for while reading it, because these are the constraints the
-- whole privacy model rests on (§54, §73):
--   * there is no `customers`, `audience_members`, `segment_members`,
--     `partner_transactions` or `cross_partner_identity` table;
--   * no column anywhere is named `partner_user_id`;
--   * `attribution_tokens.token_hash` is BYTEA and there is no column that
--     could hold the raw token (§90);
--   * `segments` publishes a reach BUCKET, never an exact count (§72).
-- =============================================================================
-- >>> BEGIN VERBATIM SNAPSHOT
-- CreateEnum
